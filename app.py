...
import sqlite3
import requests
from flask import Flask, render_template, request, redirect, session, url_for, flash  # Importe 'flash'
...
def login():
    if request.method == 'POST':
        ...
        senha = request.form['senha']  # Corrigido aqui!
        conn = get_db_connection()
        user = conn.execute('SELECT * FROM usuarios WHERE email = ?', (email,)).fetchone() # Alterado consulta
        conn.close()
        if user and check_password_hash(user['senha'], senha): # Verificação com hash
            session['usuario'] = email
            return redirect(url_for('dashboard'))
        else:
            flash('Login falhou. Verifique o email e a senha.', 'danger') # Usando flash
